...
class char_cnn_net:
    ...
    def cnn_construct(self):
        ...
        cw1 = tf.Variable(tf.random.normal(shape=[3, 3, 1, 32], stddev=0.01), dtype=tf.float32)
        cb1 = tf.Variable(tf.random.normal(shape=[32]), dtype=tf.float32)
        ...
        cw2 = tf.Variable(tf.random.normal(shape=[3, 3, 32, 64], stddev=0.01), dtype=tf.float32)
        cb2 = tf.Variable(tf.random.normal(shape=[64]), dtype=tf.float32)
        ...
        cw3 = tf.Variable(tf.random.normal(shape=[3, 3, 64, 128], stddev=0.01), dtype=tf.float32)
        cb3 = tf.Variable(tf.random.normal(shape=[128]), dtype=tf.float32)
        ...
        fw1 = tf.Variable(tf.random.normal(shape=[3 * 3 * 128, 1024], stddev=0.01), dtype=tf.float32)
        fb1 = tf.Variable(tf.random.normal(shape=[1024]), dtype=tf.float32)
        fully1 = tf.nn.relu(tf.add(tf.matmul(conv_out, fw1), fb1))
        fully1 = tf.nn.dropout(fully1, self.keep_place)

        fw2 = tf.Variable(tf.random.normal(shape=[1024, 1024], stddev=0.01), dtype=tf.float32)
        fb2 = tf.Variable(tf.random.normal(shape=[1024]), dtype=tf.float32)
        fully2 = tf.nn.relu(tf.add(tf.matmul(fully1, fw2), fb2))
        fully2 = tf.nn.dropout(fully2, self.keep_place)

        fw3 = tf.Variable(tf.random.normal(shape=[1024, self.dataset_len], stddev=0.01), dtype=tf.float32)
        fb3 = tf.Variable(tf.random.normal(shape=[self.dataset_len]), dtype=tf.float32)
    ...
    def train(self,data_dir,save_model_path):
        ...
        X, y = self.init_data(data_dir) #初始化所有图片为20*20的67维向量
        print('success load' + str(len(y)) + 'datas')
        train_x, test_x, train_y, test_y = train_test_split(X, y, test_size=0.2, random_state=0) #用于将矩阵随机划分为训练子集和测试子集，并返回划分好的训练集测试集样本和训练集测试集标签

        out_put = self.cnn_construct()
        predicts = tf.nn.softmax(out_put) #样本属于各个类的概率
        predicts = tf.argmax(predicts, axis=1)#预测到的值
        actual_y = tf.argmax(self.y_place, axis=1)#真实的值
        accuracy = tf.reduce_mean(tf.cast(tf.equal(predicts, actual_y), dtype=tf.float32)) #评估性能，准确率
        cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=out_put, labels=self.y_place))
        opt = tf.train.AdamOptimizer(learning_rate=0.001) #是一个寻找全局最优点的优化算法，引入了二次方梯度校正
        ...
        with tf.compat.v1.Session() as sess:
            ...
            while True:
                train_index = np.random.choice(len(train_x), self.batch_size, replace=False) # 是从a 中以概率P，随机选择3个, p没有指定的时候相当于是一致的分布
    ...
    def test(self,x_images,model_path):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            print('输入的model_path是:',model_path)
            saver.restore(sess, model_path)  #报错
    ...
    def init_data(self,dir):
        ...
        for file in files:
            ...
            vector_y = [0 for i in range(len(self.dataset))] #len(dataset)=67
            index_y = self.dataset.index(dir_name)
            vector_y[index_y] = 1
            y.append(vector_y)

        X = np.array(X) #图片矩阵
        y = np.array(y).reshape(-1, self.dataset_len) #图片标签
        return X, y
...
if __name__ == '__main__':
    ...
    train_model_path = os.path.join(cur_dir, '..\\model\\char_recongnize\\trainmodel.ckpt')
    model_path = os.path.join(cur_dir,'..\\model\\char_recongnize\\model.ckpt-510')
    ...
    if train_flag == 1:
        # 训练模型
        net.train(data_dir,train_model_path) #得到一个67维的向量，每个图片矩阵为20*20
    else:
        # 测试部分
        test_X = net.init_testData(test_dir)
        text = net.test(test_X,model_path)
        print(text)
#tf.Session 修改为 tf.compat.v1.Session()